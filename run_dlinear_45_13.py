...
for key in ['13', '45']:
    ...
    if key == '13':
        best_config = {'in_len': 83, 'out_len': 3, 'kernel_size': 22, 'const_init': False, 'lr': 0.000520496974095313, 'batch_size': 128} 
    elif key == '45':
        best_config = {'in_len': 104, 'out_len': 9, 'kernel_size': 31, 'const_init': False, 'lr': 0.000288169695307554, 'batch_size': 32}
    ...
    best_model = build_fit_dlinear_model_return(best_config, callbacks=[early_stopper])
    ...
    # save dfY
    dfY.to_csv(f'dlinear_{key}.csv', index=True)
